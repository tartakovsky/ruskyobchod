...
if ( ! function_exists( 'catch_store_breadcrumb' ) ) :
	function catch_store_breadcrumb() {
		$show_on_home = get_theme_mod( 'catch_store_breadcrumb_on_homepage' );

		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'catch-store' ); // text for the 'Home' link
		/* translators: 1: before text/html, 2: after text/html. */
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'catch-store' ); // text for a category page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'catch-store' ); // text for a search results page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'catch-store' ); // text for a tag page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'catch-store' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'catch-store' ); // text for the 404 page
		...
		if ( is_front_page() ) {
			if ( $show_on_home ) {
				...
				<div class="breadcrumb-area custom">
					<div class="wrapper">
						...
						echo $linkBefore . '<a href="' . esc_url( $homeLink ) . '">' . $text['home'] . '</a>' . $linkAfter; // WPCS: XSS OK.
			...
			if ( is_home() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'catch-store' ) ) ) ), esc_html( get_the_time( __( 'Y', 'catch-store' ) ) ) ) ; // WPCS: XSS OK.


				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'catch-store' ) ), get_the_time( __( 'n', 'catch-store' ) ) ) ), esc_html( get_the_time( __( 'F', 'catch-store' ) ) ) ); // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'd', 'catch-store' ) ) ) . $after; // WPCS: XSS OK.

			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'catch-store' ) ) ) ), esc_html( get_the_time( __( 'Y', 'catch-store' ) ) ) ) ; // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'F', 'catch-store' ) ) ) . $after; // WPCS: XSS OK.


			} elseif ( is_year() ) {
				echo $before . esc_html( get_the_time( __( 'Y', 'catch-store' ) ) ) . $after; // WPCS: XSS OK.
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'catch-store' ), absint( max( $paged, $page ) ) );
