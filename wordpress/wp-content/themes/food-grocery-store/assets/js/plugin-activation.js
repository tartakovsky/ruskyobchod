jQuery(document).ready(function($) {
    'use strict';
    var this_obj = food_grocery_store_plugin_activate_plugin;
    ...
    $('#product_addons_editor #install_ibtana-visual-editor .plugin-activation-redirect a.button').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.food-grocery-store-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message food-grocery-store-plugin-install');
                button.addClass('button-primary food-grocery-store-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.food-grocery-store-recommended-plugins .food-grocery-store-activation-note').css('display','block');
    ...
    $(document).on('click', '.food-grocery-store-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .food-grocery-store-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .food-grocery-store-recommended-plugins').css('display','none');
    });
