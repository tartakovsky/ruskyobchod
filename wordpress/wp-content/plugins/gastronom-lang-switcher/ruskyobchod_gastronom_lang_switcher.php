...
// Override locale for WooPayments scripts only
add_filter('wcpay_locale', function() { return 'sk_SK'; });
        ...
        foreach ($wp_scripts->registered as $handle => $script) {
            ...
            if (!empty($script->extra['data'])) {
                $wp_scripts->registered[$handle]->extra['data'] = str_replace(
                    ['"locale":"ru"', '%22locale%22%3A%22ru%22'],
                    ['"locale":"sk"', '%22locale%22%3A%22sk%22'],
                    $script->extra['data']
                );
            }
...
// Force WooPayments to show Slovak labels
...
function gls_enqueue_scripts() {
    wp_enqueue_style('gls-style', plugin_dir_url(__FILE__) . 'gls-style.css', [], '6.6');
    wp_enqueue_script('gls-script', plugin_dir_url(__FILE__) . 'gls-script.js', [], '6.6', true);
}
...
function gls_footer_credit() {
    echo '<div class="gls-footer-credit">&copy; ' . date('Y') . ' Gastronom — Ruský obchod s potravinami</div>';
}
