	...
	if (is_array($response) && !empty(json_decode($response['body'],true))) {
		...
		return array(
			'validation_errors' => array(
				array(
					'attribute' => 'Error',
					'reason' => (isset($response->errors) ? $response->errors['http_request_failed'][0] : 'Skontrolujte nastavenie pluginu v Nastavenia/ePodací hárok a vyplňte chýbajúce polia.')
				)
			)
		);
	}
