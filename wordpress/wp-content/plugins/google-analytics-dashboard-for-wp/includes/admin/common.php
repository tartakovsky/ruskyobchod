 ...
 * @return bool
 */
function exactmetrics_is_own_admin_page() {
	...
	if ( 'dashboard_page_exactmetrics-getting-started' === get_current_screen()->id ) {
		return true;
	}
...
function hide_non_exactmetrics_warnings() {
	// Bail if we're not on a ExactMetrics screen.
	if ( empty( $_REQUEST['page'] ) || strpos( sanitize_text_field( $_REQUEST['page'] ), 'exactmetrics' ) === false ) {
		return;
	}
 ...
 * @return mixed
 */
function exactmetrics_year_in_review_check_for_ecommerce( $data, $name, $report ) {

	if ( $name === 'yearinreview' && ! class_exists( 'ExactMetrics_eCommerce' ) ) {
		unset( $data['data']['ecommerce'] );
	}

	return $data;
}
add_filter( 'exactmetrics_vue_reports_data', 'exactmetrics_year_in_review_check_for_ecommerce', 3, 10 );
...
function exactmetrics_yearinreview_dates() {
	...
	if ( function_exists( 'exactmetrics_is_debug_mode' ) && exactmetrics_is_debug_mode() ) {
		$show_report = true;
	}
...
function exactmetrics_get_sitei() {
