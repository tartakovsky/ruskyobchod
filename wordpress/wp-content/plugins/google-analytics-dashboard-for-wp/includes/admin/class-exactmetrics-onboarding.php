...
/**
 * ExactMetrics Onboarding Class
 *
 * @package ExactMetrics
 * @author David Paternina
 */
class ExactMetrics_Onboarding {
	...
	public function get_info( $request ) {
		...
		if ( ! function_exists( 'exactmetrics_get_addons' ) ) {
			require_once EXACTMETRICS_PLUGIN_DIR . 'includes/admin/pages/addons.php';
		}

		if ( ! class_exists( 'ExactMetrics_Rest_Routes' ) ) {
		...
		if ( exactmetrics_is_pro_version() ) {
			...
			$licensing = new ExactMetrics_License();
		} else {
			require_once EXACTMETRICS_PLUGIN_DIR . 'lite/includes/license-compat.php';
			$licensing = new ExactMetrics_License_Compat();
		}

		$rest_routes = new ExactMetrics_Rest_Routes();
		...
		$mi_info = array(
			'existing_settings'     => array(
				'site_type'             => exactmetrics_get_option( 'site_type', 'business' ),
				'extensions_of_files'   => exactmetrics_get_option( 'extensions_of_files' ),
				'affiliate_links'       => exactmetrics_get_option( 'affiliate_links' ),
				'view_reports'          => exactmetrics_get_option( 'view_reports' ),
				'automatic_updates'     => exactmetrics_get_option( 'automatic_updates' ),
				'anonymous_data'        => exactmetrics_get_option( 'anonymous_data' ),
				'verified_automatic'    => exactmetrics_get_option( 'verified_automatic' ),
			),
			'plugin_info'          => array(
				'plugin_version'       => EXACTMETRICS_VERSION,
				'is_pro'               => exactmetrics_is_pro_version() ? 1 : 0,
				'installed_version'    => exactmetrics_get_option( 'installed_version' ),
				'installed_date'        => exactmetrics_get_option( 'installed_date' ),
				'site_license'  => array(
	...
	public function store_settings( $request ) {
		...
		if ( $request->get_param('license') && class_exists( 'ExactMetrics' ) ) {
			$license = $request->get_param('license');
			$option                = $is_network ? ExactMetrics()->license->get_network_license() : ExactMetrics()->license->get_site_license();
			...
			$is_network ? ExactMetrics()->license->set_network_license( $option ) : ExactMetrics()->license->set_site_license( $option );
		...
		if ( ! empty( $auth ) ) {
			if( isset( $auth['is_authed'] ) &&  true === $auth['is_authed'] ) {
				...
				$existing = $is_network ? ExactMetrics()->auth->get_network_analytics_profile() : ExactMetrics()->auth->get_analytics_profile();
	...
	public function get_connect_url( $request ) {
		...
		if ( empty( $key ) ) {
			return new WP_Error(
				'missing_key',
				esc_html__( 'Please enter your license key to connect.', 'google-analytics-dashboard-for-wp' ),
				array( 'status' => 400 )
			);
		}
		$url_data = ExactMetrics_Connect::generate_connect_url_data( $key, $network );
		if ( empty( $url_data ) ) {
			return new WP_Error(
				'url_generation_failed',
				esc_html__( 'Failed to generate connect URL.', 'google-analytics-dashboard-for-wp' ),
				array( 'status' => 500 )
			);
		}
	...
	public function set_license_key( $request ) {
		...
		$verify = exactmetrics_perform_remote_request( 'verify-key', array( 'tgm-updater-key' => $license_key ) );
		if ( ! $verify ) {
			return new WP_REST_Response(
				array(
