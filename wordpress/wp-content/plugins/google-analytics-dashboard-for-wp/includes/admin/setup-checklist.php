...
/**
 * Class ExactMetrics_Setup_Checklist
 */
class ExactMetrics_Setup_Checklist {

	/**
	 * ExactMetrics_Setup_Checklist constructor.
	 */
	public function __construct() {
		add_action( 'wp_ajax_exactmetrics_vue_get_setup_checklist', array( $this, 'ajax_get_setup_checklist' ) );
		add_action( 'wp_ajax_exactmetrics_vue_setup_checklist_click_track', array(
			$this,
			'ajax_button_click_track'
		) );
		add_action( 'wp_ajax_exactmetrics_generate_setup_wizard_url', array( $this, 'ajax_generate_setup_wizard_url' ) );
	}
	...
	private function default_checklist() {
		return array(
			'step_1_install_exactmetrics' => true,
			'step_1_connect_exactmetrics' => array(
	...
	public function ajax_button_click_track() {
		...
		update_option( 'exactmetrics_setup_checklist', $checklist );
	...
	public function get_menu_count() {
		...
		if ( $milestone_left > 0 ) {
			return '<span class="exactmetrics-menu-notification-indicator update-plugins exactmetrics-setup-checklist-menu-indicator">' . $milestone_left . '</span>';
		}
	...
	private function get_checklist_data() {
		...
		if ( exactmetrics_get_v4_id_to_output() ) {
			$checklist['step_1_connect_exactmetrics']['launch_setup_wizard'] = true;
			$checklist['step_1_connect_exactmetrics']['select_a_property']   = true;
			$checklist['step_1_connect_exactmetrics']['ga_receiving_data']   = true;
		}
		...
		$notes_count = wp_count_posts( 'exactmetrics_note' );
	...
	public function ajax_generate_setup_wizard_url() {
		...
		if ( ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json_error( array(
				'message' => esc_html__( 'You don\'t have permission to perform this action.', 'google-analytics-dashboard-for-wp' ),
			) );
		}
		// Use the existing function to get the onboarding URL
		$onboarding_url = exactmetrics_get_onboarding_url();
