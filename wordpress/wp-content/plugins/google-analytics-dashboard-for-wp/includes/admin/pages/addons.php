...
function exactmetrics_get_addons_data( $key ) {
	...
	if ( exactmetrics_is_pro_version() && $key ) {
		$addons = exactmetrics_perform_remote_request( 'get-addons-data-v600', array( 'tgm-updater-key' => $key ) );
	} else {
...
function exactmetrics_get_all_addons_data() {
	...
	return exactmetrics_perform_remote_request( 'verify-key', $body );
}

function exactmetrics_get_addon( $installed_plugins, $addons_type, $addon, $slug ) {
	$active          = false;
	$installed       = false;
