 ...
 * @package ExactMetrics_Site_Notes
 */
 ...
 * @since 1.0.0
 */
class ExactMetrics_Site_Notes_DB_Base
{
	...
	private function create_post_type()
	{
		...
		register_post_type('exactmetrics_note', $args);
	}
	...
	private function create_taxonomy()
	{
		...
		register_taxonomy('exactmetrics_note_category', array('exactmetrics_note'), $args);
	}

	public function insert_default_categories()
	{
		if (get_option('exactmetrics_sitenotes_installed')) {
			return;
		}
	...
	{
		$term = get_term($id, 'exactmetrics_note_category');
		if (is_wp_error($term)) {
			return false;
		}
	...
	{
		if (empty($data['note'])) {
			return new WP_Error(400, __('Your Site Note Cannot be Empty', 'google-analytics-dashboard-for-wp'));
		}
		$note_post = array(
		...
		if (!empty($data['category'])) {
			if ($category_name = $this->get_category_name($data['category'])) {
				wp_set_object_terms($post_id, $data['category'], 'exactmetrics_note_category');
				update_post_meta($post_id, '_category', $category_name);
			}
		} else {
			wp_set_object_terms($post_id, array(), 'exactmetrics_note_category');
	...
	{
		if (isset($data['id'])) {
			$created_term = wp_update_term($data['id'], 'exactmetrics_note_category', array('name' => $data['name']));
		} else {
			$created_term = wp_insert_term($data['name'], 'exactmetrics_note_category');
		}
	...
	public function get($post_id)
	{
		...
		if (!$post) {
			return new WP_Error(400, __('Note not found', 'google-analytics-dashboard-for-wp'));
		}

		$categories = wp_get_object_terms($post->ID, 'exactmetrics_note_category');
	...
	public function get_items($args = array(), $count = false)
	{
		...
		if (!empty($args['category'])) {
			$query_args['tax_query'] = array(
				array(
					'taxonomy' => 'exactmetrics_note_category',
					'field'    => 'term_id',
					'terms'    => $args['category'],
				),
			);
	...
	public function delete_note( $note_id = 0 )
	{
		...
		if ( ! empty( $ga4_annotation_id ) ) {
			...
			$controller = ExactMetrics_SiteNotes_Controller::get_instance();
	...
	public function delete_category($id = 0)
	{
		...
		return wp_delete_term($id, 'exactmetrics_note_category');
	}
	...
	public function create_note_in_ga4( $post_id, $data )
	{
		...
		if ( $is_update ) {
			...
			if ( ! empty( $existing_ga4_id ) ) {
				...
				$api = new ExactMetrics_API_Request( 'analytics/reports/annotations/update', $api_options, 'POST' );
