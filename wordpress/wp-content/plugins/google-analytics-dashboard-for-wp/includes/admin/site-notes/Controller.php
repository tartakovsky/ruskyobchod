...
class ExactMetrics_SiteNotes_Controller {

	public static $instance;

	/**
	 * @var ExactMetrics_Site_Notes_DB_Base
	 */
	private $db;
	...
	public static function get_instance() {
		if (!isset(self::$instance) && !(self::$instance instanceof ExactMetrics_SiteNotes_Controller)) {
			self::$instance = new ExactMetrics_SiteNotes_Controller();
		}
	...
	public function load_dependencies() {
		require_once EXACTMETRICS_PLUGIN_DIR . 'includes/admin/site-notes/Database.php';
		$this->db = new ExactMetrics_Site_Notes_DB_Base();
	}

	public function add_hooks() {
		add_action('init', array($this->db, 'install'));
		add_action('wp_ajax_exactmetrics_vue_get_notes', array($this, 'get_notes'));
		add_action('wp_ajax_exactmetrics_vue_get_note', array($this, 'get_note'));
		add_action('wp_ajax_exactmetrics_vue_get_categories', array($this, 'get_categories'));
		add_action('wp_ajax_exactmetrics_vue_save_note', array($this, 'save_note'));
		add_action('wp_ajax_exactmetrics_vue_save_category', array($this, 'save_category'));
		add_action('wp_ajax_exactmetrics_vue_trash_notes', array($this, 'trash_notes'));
		add_action('wp_ajax_exactmetrics_vue_restore_notes', array($this, 'restore_notes'));
		add_action('wp_ajax_exactmetrics_vue_delete_notes', array($this, 'delete_notes'));
		add_action('wp_ajax_exactmetrics_vue_delete_categories', array($this, 'delete_categories'));
		add_action( 'wp_ajax_exactmetrics_vue_export_notes', array( $this, 'export_notes_to_ga4' ) );
		add_action( 'wp_ajax_exactmetrics_vue_import_notes', array( $this, 'import_notes_from_ga4' ) );
		...
		add_filter('exactmetrics_report_overview_data', array($this, 'prepare_data_overview_chart'));
		add_filter('exactmetrics_report_traffic_sessions_chart_data', array($this, 'prepare_traffic_sessions_chart_data'), 10, 4);
		add_action('save_post', array($this, 'save_custom_fields'));
		add_filter('exactmetrics_gutenberg_tool_vars', array($this, 'add_categories_to_editor'));
		...
		add_action('exactmetrics_after_exclude_metabox', array($this, 'add_metabox_contents'), 11, 2);
	...
	public function get_notes() {
		...
		if ( ! current_user_can( 'exactmetrics_view_dashboard' ) && ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to view notes.", 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
	...
	public function get_categories() {
		...
		if ( ! current_user_can( 'exactmetrics_view_dashboard' ) && ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to view notes categories.", 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
	...
	public function save_note() {
		...
		if ( ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to update notes.", 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
	...
	public function save_category() {
		...
		if ( ! current_user_can( 'exactmetrics_save_settings' ) ) {
			wp_send_json(
				array(
					'published' => false,
					'message' => __( "You don't have permission to update categories.", 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
	...
	public function change_restore_note_status($new_status, $post_id, $previous_status) {
		if ('exactmetrics_note' !== get_post_type($post_id)) {
			return $new_status;
		}
	...
	public function export_notes() {
		...
		if ('exactmetrics_export_notes' !== $_POST['exactmetrics_action']) {
			return;
		}
		...
		header('Content-Disposition: attachment; filename=exactmetrics-notes-export-' . wp_date('m-d-Y') . '.csv');
		header("Expires: 0");
		...
		foreach ($items['items'] as $item) {
			$item_media = __('NA', 'google-analytics-dashboard-for-wp');
	...
	public function export_notes_to_ga4() {
		if (
			! isset( $_POST['action'] ) ||
			'exactmetrics_vue_export_notes' !== $_POST['action']
		) {
			return;
		}
		...
		if ( empty( $annotations ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'No annotations data provided.', 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
		...
		if (
			! ( ExactMetrics()->auth->is_authed() || ExactMetrics()->auth->is_network_authed() )
		) {
			wp_send_json_error(
				array(
					'message' => __( 'You must be properly authenticated with ExactMetrics to export annotations.', 'google-analytics-dashboard-for-wp' ),
				)
			);
		}
		...
		if (
			! ExactMetrics()->auth->is_authed() &&
			ExactMetrics()->auth->is_network_authed()
		) {
			$api_options['network'] = true;
		}

		// Create API request.
		$api = new ExactMetrics_API_Request( 'analytics/reports/annotations/', $api_options, 'POST' );
		...
		exactmetrics_update_option( 'site_notes_export_synced', 1 );

		// Return success response.
		wp_send_json_success(
			array(
				'message' => __( 'Annotations exported successfully.', 'google-analytics-dashboard-for-wp' ),
				'data'    => $response,
			)
		);
	}
	...
	public function deleted_note_from_ga4_single($ga4_annotation_id) {
		...
		if (!(ExactMetrics()->auth->is_authed() || ExactMetrics()->auth->is_network_authed())) {
			return new WP_Error('not_authenticated', __('You must be properly authenticated with ExactMetrics to delete annotations.', 'google-analytics-dashboard-for-wp'));
		}
		...
		$api = new ExactMetrics_API_Request('analytics/reports/annotations/delete', $api_options, 'POST');
	...
	public function import_notes_from_ga4() {
		if (
			! isset( $_POST['action'] ) ||
			'exactmetrics_vue_import_notes' !== $_POST['action']
		) {
			return;
		}
		...
		if (
			...
			wp_send_json_error(
				array(
					'message' => esc_html__(
						'You must be properly authenticated with ExactMetrics to import annotations.',
						'google-analytics-dashboard-for-wp'
					),
				)
			);
		...
		$api = new ExactMetrics_API_Request(
		...
		if ( $skipped_count > 0 ) {
			$message .= ' ' . sprintf(
				__( '%d annotations were skipped (already exist).', 'google-analytics-dashboard-for-wp' ),
				$skipped_count
			);
		}
		...
		exactmetrics_update_option( 'site_notes_import_synced', 1 );
	...
	public function register_meta() {
		...
		register_post_meta(
			'',
			'_exactmetrics_sitenote_category',
			[
				'auth_callback' => '__return_true',
				'default'       => 0,
				'show_in_rest'  => true,
	...
	public function save_custom_fields($current_post_id) {
		if (!isset($_POST['exactmetrics_metabox_nonce']) || !wp_verify_nonce($_POST['exactmetrics_metabox_nonce'], 'exactmetrics_metabox')) {
			return;
		}

		if ('exactmetrics_note' === get_post_type($current_post_id) || 'publish' !== get_post_status($current_post_id)) {
			return;
		}

		$active = intval(isset($_POST['_exactmetrics_sitenote_active']) ? $_POST['_exactmetrics_sitenote_active'] : 0);
		update_post_meta($current_post_id, '_exactmetrics_sitenote_active', $active);

		if (!$active) {
			delete_post_meta($current_post_id, '_exactmetrics_sitenote_note');
			delete_post_meta($current_post_id, '_exactmetrics_sitenote_category');
			delete_post_meta($current_post_id, '_exactmetrics_sitenote_id');
			return;
		}

		$note = isset($_POST['_exactmetrics_sitenote_note']) ? esc_html($_POST['_exactmetrics_sitenote_note']) : '';
		update_post_meta($current_post_id, '_exactmetrics_sitenote_note', $note);

		$category = isset($_POST['_exactmetrics_sitenote_category']) ? intval($_POST['_exactmetrics_sitenote_category']) : 0;
		if ($category) {
			update_post_meta($current_post_id, '_exactmetrics_sitenote_category', $category);
		}
	}

	public function create_note_with_post($post_ID) {
		if ('exactmetrics_note' === get_post_type($post_ID) || 'publish' !== get_post_status($post_ID)) {
			return;
		}

		$active = get_post_meta($post_ID, '_exactmetrics_sitenote_active', true);

		if (!$active) {
			delete_post_meta($post_ID, '_exactmetrics_sitenote_note');
			delete_post_meta($post_ID, '_exactmetrics_sitenote_category');
			delete_post_meta($post_ID, '_exactmetrics_sitenote_id');
			return;
		}

		$note = get_post_meta($post_ID, '_exactmetrics_sitenote_note', true);

		$category = get_post_meta($post_ID, '_exactmetrics_sitenote_category', true);

		$note_id = get_post_meta($post_ID, '_exactmetrics_sitenote_id', true);
		...
		update_post_meta($post_ID, '_exactmetrics_sitenote_id', $created_note_id);
	}
	...
	public function add_metabox_contents($skipped, $post) {
		$sitenote_active = get_post_meta($post->ID, '_exactmetrics_sitenote_active', true);
		$sitenote_note = get_post_meta($post->ID, '_exactmetrics_sitenote_note', true);
		$sitenote_category = get_post_meta($post->ID, '_exactmetrics_sitenote_category', true);
		...
		$categories = $this->db->get_categories($args); ?>
		<div class="exactmetrics-metabox" id="exactmetrics-metabox-site-notes">
			<div class="exactmetrics-metabox-input exactmetrics-metabox-input-checkbox">
				<label class="">
					<input type="checkbox" name="_exactmetrics_sitenote_active" value="1" <?php checked($sitenote_active); ?>>
					<span class="exactmetrics-metabox-input-checkbox-label"><?php _e('Add a Site Note', 'google-analytics-dashboard-for-wp'); ?></span>
				</label>
			</div>

			<div id="site-notes-active-container" class="<?php echo (!$sitenote_active ? 'hidden' : ''); ?>">
				<div class="exactmetrics-metabox-input exactmetrics-metabox-textarea">
					<textarea name="_exactmetrics_sitenote_note" rows="3"><?php echo esc_textarea($sitenote_note); ?></textarea>
				</div>

				<div class="exactmetrics-metabox-input exactmetrics-metabox-select">
					<label>
						<?php _e('Category', 'google-analytics-dashboard-for-wp'); ?>
						<select name="_exactmetrics_sitenote_category">
...
<?php
	...
	public function load_metabox_assets() {
		...
		wp_register_script('exactmetrics-admin-metabox-sitenotes-script', plugins_url('assets/js/admin-metabox-sitenotes.js', EXACTMETRICS_PLUGIN_FILE), array('jquery'), exactmetrics_get_asset_version());
		wp_enqueue_script('exactmetrics-admin-metabox-sitenotes-script');
	}
...
ExactMetrics_SiteNotes_Controller::get_instance()->run();
