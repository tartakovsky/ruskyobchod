...
final class ExactMetrics_Notice_Admin {
	...
	public function display_inline_notice( $name, $title, $message, $type = 'success', $is_dismissible = false, $args = array() ) {
		...
		?>
		<div
			class="exactmetrics-notice <?php echo 'exactmetrics-' . esc_attr( $type ) . '-notice' . esc_attr($dismissible); ?>"
			data-notice="<?php echo esc_attr( $name ); ?>">
			<div
				class="exactmetrics-notice-icon <?php echo 'exactmetrics-' . esc_attr( $type ) . '-notice-icon' ?>">
			</div>
			<div
				class="exactmetrics-notice-text <?php echo 'exactmetrics-' . esc_attr( $type ) . '-notice-text' ?>">
				<?php
				// Title
				if ( ! empty ( $title ) ) {
					?>
				...
				if ( ! empty( $message ) ) {
					if ( empty( $args['skip_message_escape'] ) ) {
						...
						?>
						<p class="exactmetrics-notice-message"><?php echo $message; // phpcs:ignore ?></p>
						<?php
					}
				}
