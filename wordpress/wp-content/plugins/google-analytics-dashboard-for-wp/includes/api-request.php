...
final class ExactMetrics_API_Request {
	...
	public function request( $extra_params = [] ) {
		...
		if ( 200 != $response_code && 204 != $response_code) {
			...
			if ( empty( $response_code ) ) {
				...
				$message = sprintf(
					esc_html__( 'Oops! We encountered an error. Please wait a few minutes and try again. If the issue persists, please %1$scontact our support%2$s team.', 'google-analytics-dashboard-for-wp' ),
					'<a target="_blank" href="' . exactmetrics_get_url( 'notice', 'unknown-api-error', 'https://www.exactmetrics.com/my-account/support/' ) . '">',
					'</a>'
				);
			...
			if ( empty( $response_body ) || ( empty( $response_body['message'] ) && empty( $response_body['error'] ) ) ) {
				...
				$message = sprintf(
					esc_html__( 'Oops! We ran into a problem. Please try again in a few minutes. If the issue persists please %1$scontact our support%2$s team. Error: API returned a %3$s%4$s%5$s response.', 'google-analytics-dashboard-for-wp' ),
					'<a target="_blank" href="' . exactmetrics_get_url( 'notice', 'unknown-api-error', 'https://www.exactmetrics.com/my-account/support/' ) . '">',
					'</a>',
					'<strong>',
					$response_code,
			...
			if ( ! empty( $response_body['message'] ) ) {
				...
				$message = sprintf(
					esc_html__( 'Oops! We ran into a problem. Please try again in a few minutes. If the issue persists please %1$scontact our support%2$s team. Error: API returned a %3$s%4$d: %5$s%6$s', 'google-analytics-dashboard-for-wp' ),
					'<a target="_blank" href="' . exactmetrics_get_url( 'notice', 'unknown-api-error', 'https://www.exactmetrics.com/my-account/support/' ) . '">',
					'</a>',
					'<strong>',
					$response_code,
		...
		if ( ! empty( $this->tt ) ) {
			if ( empty( $response_body['tt'] ) || ! hash_equals( $this->tt, $response_body['tt'] ) ) {
				...
				// Translators: Support link tag starts with url and Support link tag ends.
				$message = sprintf(
					esc_html__( 'Oops! We ran into a problem. Please try again in a few minutes. If the issue persists please %1$scontact our support%2$s team. Error: Improper API Request.', 'google-analytics-dashboard-for-wp' ),
	...
	private function is_blocked( $url = '' ) {
		...
		if ( defined( 'AIRMDE_VER' ) && ! empty( $Airplane_Mode_Core ) && $Airplane_Mode_Core->enabled() ) {
			return new WP_Error( 'api-error', __( 'Oops! The API was unreachable because the plugin, Airplane Mode is active. Please disable and try again.', 'google-analytics-dashboard-for-wp' ) );
		}
