...
/**
 * ExactMetrics PPC Tracking Ecommerce tracking base class.
 *
 * @access public
 * @since 9.7.0
 *
 * @package ExactMetrics_PPC_Tracking
 * @subpackage Ecommerce_Providers
 * @author  David Paternina
 */
abstract class ExactMetrics_Ads_Tracking_Ecommerce_Tracking {
	 ...
	 * Add providers
	 *
	 * @var ExactMetrics_Ads_Tracking_Provider[]
	 */
	private $providers;

	/**
	 * @param $ad_providers ExactMetrics_Ads_Tracking_Provider[]
	 */
	...
	public function add_frontend_hooks() {
		add_action( 'exactmetrics_frontend_tracking_gtag_after_pageview', [ $this, 'insert_conversion_code' ] );
	}
	 ...
	 * @param array $data
	 * @param ExactMetrics_Ads_Tracking_Ecommerce_Tracking $ecommerce_provider
	 *
	 * @return void
	 */
	...
	public function insert_conversion_code() {
		if ( apply_filters( 'exactmetrics_ppp_tracking_is_conversion_page', $this->do_conversion_checks() ) ) {
			...
			foreach ( $this->providers as $provider ) {
				...
				if ( apply_filters( "exactmetrics_ppc_tracking_track_already_tracked_{$provider->get_provider_id()}", $this->already_tracked_frontend( $provider->get_provider_id() ) ) ) {
					continue;
				}
	 ...
	 *
	* @since 9.7.0
	 * @return ExactMetrics_Ads_Tracking_Provider[]
	 */
	...
	{
		$tracked = $this->get_order_meta( $order_id, "_exactmetrics_ppc_tracking_conversion_tracked_{$provider_id}" );
	...
	{
		$this->store_extra_data( $order_id, [ "_exactmetrics_ppc_tracking_conversion_tracked_{$provider_id}" => 'yes' ] );
	}
	...
	public function server_order_saved( $order_id, $data = [] )
	{
		foreach ( $this->get_ad_providers() as $provider ) {
			...
			$this->store_extra_data(
				...
				array_merge(
					[
						'_exactmetrics_ppc_tracking_user_agent' => $this->get_user_agent(),
						'_exactmetrics_ppc_tracking_user_ip'    => $this->get_user_ip(),
					],
					$ad_provider_data
				)
			);
...
if ( !class_exists('ExactMetrics_PPC_Tracking_Ecommerce_Tracking') ) {
	// class_alias( 'ExactMetrics_Ads_Tracking_Ecommerce_Tracking', 'ExactMetrics_PPC_Tracking_Ecommerce_Tracking' );
}