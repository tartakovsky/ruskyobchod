...
/**
 * ExactMetrics PPC Tracking - LifterLMS tracking class.
 *
 * @access public
 * @since 9.7.0
 *
...
class ExactMetrics_Ads_Tracking_Ecommerce_Lifter_LMS extends ExactMetrics_Ads_Tracking_Ecommerce_Tracking
{
	/**
	 * LifterLMS order
	 *
	...
	public function is_active() {
		return function_exists( 'LLMS' ) &&
			...
			apply_filters( 'exactmetrics_ppc_tracking_track_conversion_lifterlms', true );
	}
	...
	public function already_tracked_frontend( $provider_id ) {
		...
		$tracked = get_post_meta( $this->order->id, "_exactmetrics_ppc_tracking_conversion_tracked_{$provider_id}", true );
	...
	public function mark_order_tracked_frontend( $provider_id ) {
		...
		update_post_meta( $this->order->id, "_exactmetrics_ppc_tracking_conversion_tracked_{$provider_id}", 'yes' );
	}
