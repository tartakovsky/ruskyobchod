...
/**
 * ExactMetrics PPC Tracking - EDD tracking class.
 *
 * @access public
 * @since 9.7.0
 *
...
class ExactMetrics_Ads_Tracking_Ecommerce_EDD extends ExactMetrics_Ads_Tracking_Ecommerce_Tracking
{
	/**
	 * EDD Payment/order
	 *
	...
	public function is_active() {
		return class_exists( 'Easy_Digital_Downloads' ) && apply_filters( 'exactmetrics_ppc_tracking_track_conversion_edd', true );
	}
	...
	public function already_tracked_frontend( $provider_id ) {
		...
		$tracked = $this->payment->get_meta( "_exactmetrics_ppc_tracking_conversion_tracked_{$provider_id}", true );
	...
	public function mark_order_tracked_frontend( $provider_id ) {
		...
		$this->payment->update_meta("_exactmetrics_ppc_tracking_conversion_tracked_{$provider_id}", 'yes' );
	}
	...
	public function server_event_purchase( $order_id, $new_status, $old_status ) {
		...
		$skip_renewals = apply_filters( 'exactmetrics_ppc_tracking_skip_renewals', true );
		...
		$data = [
			...
			'user_agent' => $this->get_order_meta( $order_id, '_exactmetrics_ppc_tracking_user_agent' ),
