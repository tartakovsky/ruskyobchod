...
/**
 * ExactMetrics PPC Tracking - Restrict Content Pro tracking class.
 *
 * @access public
 * @since 9.7.0
 *
...
class ExactMetrics_Ads_Tracking_Ecommerce_RCP extends ExactMetrics_Ads_Tracking_Ecommerce_Tracking
{
	/**
	 * RCP Payment
	 *
	...
	public function is_active() {
		return class_exists( 'Restrict_Content_Pro' ) &&
			...
			apply_filters( 'exactmetrics_ppc_tracking_track_conversion_rcp', true );
	}
	...
	function already_tracked_frontend( $provider_id ) {
		...
		$tracked = $rcp_payments->get_meta(
			...
			"_exactmetrics_ppc_tracking_conversion_tracked_{$provider_id}",
			true
		);
	...
	public function server_event_begin_checkout() {
		...
		$level = apply_filters('exactmetrics_ppc_checkout_tracking_rcp_level', $default_level, $rcp_levels);
	...
	public function server_event_purchase($payment_id) {
		...
		$data = [
			...
			'user_ip'    => $this->get_order_meta( $payment_id, '_exactmetrics_ppc_tracking_user_ip' ),
			'phone'      => '',
			...
			'state'      => '',
			'country'    => '',
			'user_agent' => $this->get_order_meta( $payment_id, '_exactmetrics_ppc_tracking_user_agent' ),
