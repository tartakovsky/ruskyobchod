...
check_ok 'mu-plugins contains no public backup source artifacts' ssh -p "$REMOTE_PORT" "$REMOTE_HOST" "test -z \"\$(find '$REMOTE_ROOT/wp-content/mu-plugins' -maxdepth 1 -type f \\( -name '*.off' -o -name '*.off.*' -o -name '*.bak' -o -name '*.bak-*' -o -name '*.old' -o -name '*~' \\) -print -quit)\""
