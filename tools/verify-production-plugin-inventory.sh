...
REMOTE_SCRIPT="/tmp/rusky-verify-production-plugin-inventory-$$.php"
...
scp -P "$REMOTE_PORT" "$ROOT_DIR/tools/verify-production-plugin-inventory.php" "$REMOTE_HOST:$REMOTE_SCRIPT" >/dev/null
ssh -p "$REMOTE_PORT" "$REMOTE_HOST" "php '$REMOTE_SCRIPT' '$REMOTE_ROOT'"

echo 'Production plugin inventory verification complete.'
